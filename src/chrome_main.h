// To ease upgrades, this file is copied from Chromium's
// chrome/app/chrome_main.cc. We need a copy because Chromium doesn't structure
// this as a h/cc combo but we need to get at the ChromeMainDelegate class to
// avoid pretty substantial code duplication (and a maintenance nightmare). To
// do this without conflicting, we only make a very minor change: we wrap most
// of this in our own namespace. Merging in updates should just be a matter of
// adding in diffs other than the namespace modification.
//
// Note that this *isn't* really a standalone file. We include it and use it
// directly from Root.cpp.
...
static int ChromeMain(int argc, char** argv);
}
#endif
